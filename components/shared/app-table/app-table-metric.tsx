...
import { formatValue, formatValueColor, IMetricData } from '@/lib';
		...
		<Card className={cn("", className)}>
			{isLoading
				...
				: <CardHeader className="relative">
					...
					<CardTitle className={cn("font-bold text-2xl 2k:text-[32px] 4k:text-5xl 8k:text-8xl", metric.key === "Gain" && formatValueColor(metric.rawValue) || metric.key === "Estimated gain after fees" && formatValueColor(metric.rawValue))}>
					...
					{metric.percentage !== undefined &&
						<CardAction className="absolute top-[-22px] right-1 2xl:top-[-20px] 2xl:right-1.5 full-hd:top-[-12px] full-hd:right-3 2k:top-[-16px] 2k:right-4 4k:top-[-24px] 4k:right-6 8k:top-[-48px] 8k:right-12">
							<Badge variant={'outline'}>
								<span className={cn("font-medium 2k:text-base 4k:text-2xl 8k:text-5xl", formatValueColor(metric.percentage))}>
